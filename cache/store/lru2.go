...
// GetValue 获取指定键的原始值（不修改 LRU 位置）
func (s *lru2Store) GetValue(key string) (Value, bool) {
	idx := hashBKRD(key) & s.mask
	s.locks[idx].Lock()
	defer s.locks[idx].Unlock()
	...
	// 先查一级缓存
	if idx1, ok1 := s.caches[idx][0].hmap[key]; ok1 {
		if s.caches[idx][0].m[idx1-1].expireAt > 0 && currentTime < s.caches[idx][0].m[idx1-1].expireAt {
			return s.caches[idx][0].m[idx1-1].v, true
		}
	}

	// 再查二级缓存
	if idx2, ok2 := s.caches[idx][1].hmap[key]; ok2 {
		if s.caches[idx][1].m[idx2-1].expireAt > 0 && currentTime < s.caches[idx][1].m[idx2-1].expireAt {
			return s.caches[idx][1].m[idx2-1].v, true
		}
	}
...
func init() {
	go func() {
		// 使用 time.NewTicker 周期性通过 time.Now() 强校准时钟
		// 消除因 time.Sleep 调度延迟产生的累积漂移
