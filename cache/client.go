...
// propagateContext 将 context.Value 中的标记转发为 gRPC outgoing metadata
func propagateContext(ctx context.Context) context.Context {
	var pairs []string
	if ctx.Value("from_peer") != nil {
		pairs = append(pairs, "from-peer", "true")
	}
	if ctx.Value("from_migration") != nil {
		pairs = append(pairs, "from-migration", "true")
	}
	if len(pairs) > 0 {
...
func (c *Client) Delete(ctx context.Context, group, key string) (bool, error) {
	...
	// Ensure a timeout if caller didn't set one
